...
import { createArk } from './index';
import type { GateSummary, AuditStats } from './types';
...
function handleTool(name: string, args: Record<string, unknown> = {}): ToolResult {
  switch (name) {
    ...
    case 'ark_status': {
      const summary: GateSummary = ark.summary();
    ...
    case 'ark_stats': {
      ...
      const stats: AuditStats = ark.stats(days);
      const text = [
...
function handleMessage(msg: JsonRpcMessage): Record<string, unknown> | null {
