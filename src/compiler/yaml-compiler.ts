...
import { NaturalLangMapper } from './natural-lang';

/**
 * YamlCompiler — Reads ark.setup.yaml and produces ParsedRules.
 */
export class YamlCompiler {
  private readonly _mapper: NaturalLangMapper;

  constructor(language: string = 'en', customMappings: Record<string, string[]> = {}) {
    this._mapper = new NaturalLangMapper(language, customMappings);
  }
  ...
  private _compileConstitution(config: ArkSetupConfig): ParsedRules {
    const blacklists: Record<string, BlacklistRule> = {};
    const contracts: Record<string, Contract> = {};
    const gates: Record<string, GateRule> = {};

    // Extract statements — handle both formats
    const constitution = config.constitution as unknown;
    let statements: string[] = [];

    if (Array.isArray(constitution)) {
      statements = constitution.filter((s): s is string => typeof s === 'string');
      ...
      if (Array.isArray(obj.statements)) {
        statements = obj.statements.filter((s): s is string => typeof s === 'string');
      }
    }
    ...
    const seqSource = (constitution && typeof constitution === 'object')
      ? (constitution as Record<string, unknown>).sequences
      : undefined;

    if (seqSource && typeof seqSource === 'object' && !Array.isArray(seqSource)) {
      for (const [seqName, seqRaw] of Object.entries(seqSource as Record<string, unknown>)) {
        const seqConfig = seqRaw as Record<string, string> | undefined;
        if (!seqConfig?.flow) continue;
  ...
  private _compileGeneralRules(config: ArkSetupConfig): ParsedRules {
    const blacklists: Record<string, BlacklistRule> = {};
    const contracts: Record<string, Contract> = {};
    const gates: Record<string, GateRule> = {};
    ...
    if (config.rules?.block && Array.isArray(config.rules.block)) {
      for (const blockRule of config.rules.block) {
        if (!blockRule || typeof blockRule !== 'object') continue;
        const br = blockRule as unknown as Record<string, unknown>;
        const patterns: RegExp[] = [];
        const patternList = br.patterns;
        if (Array.isArray(patternList)) {
          for (const p of patternList) {
            if (typeof p === 'string' && p) {
              try { patterns.push(new RegExp(p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i')); } catch { /* skip */ }
            }
          }
        }
    ...
    if (config.rules?.gates && Array.isArray(config.rules.gates)) {
      for (const gateRule of config.rules.gates) {
        if (!gateRule || typeof gateRule !== 'object') continue;
        const gr = gateRule as unknown as Record<string, unknown>;
        if (typeof gr.name === 'string' && Array.isArray(gr.actions)) {
          gates[gr.name] = {
            actions: gr.actions.filter((a): a is string => typeof a === 'string'),
  ...
  static parseSimpleYaml(source: string): ArkSetupConfig {
    ...
    return result as unknown as ArkSetupConfig;
  }
  ...
  private static _parseYamlBlock(
    ...
    while (i < lines.length) {
      ...
      if (indent < minIndent) break; // Back to parent

      const content = line.trim();
      if (content.startsWith('- ')) { break; } // Array items handled by parent
      ...
      i++;
      if (i >= lines.length) { target[key] = {}; break; }
  ...
  private static _parseScalar(raw: string): unknown {
