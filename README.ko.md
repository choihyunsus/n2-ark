...
## v3.x 업데이트 및 마이그레이션 가이드

버전 3은 AI의 자기 승인 취약점을 원천 차단하고 난독화 우회 공격을 방어하기 위한 대규모 보안 강화 업데이트입니다:

- **MCP `ark_approve` 도구 완전 제거**: 더 이상 AI가 MCP를 통해 스스로 승인을 처리할 수 없습니다. 인간의 승인은 오직 HTTP 대시보드(`localhost:9720`) 또는 CLI(`npx n2-ark approve`)를 이용한 **Out-of-band(대역 외)** 방식으로만 이루어져야 합니다.
- **난독화 정규화(Normalization) 강화**: `ark.check` 검사기가 셸 하위 표현식(`$()`, `${}`), 커맨드라인 캐럿(`^`) 치환, URL 인코딩 등을 평가 전에 미리 무력화하여 정규식 우회를 방어합니다.
- **엄격한 CORS & 루프백 차단**: 승인 서버는 CSRF 공격을 막기 위해 동일 출처 정책(Same-Origin)을 엄격히 적용하며, AI가 접근할 수 있는 모든 우회 루프백 주소(`127.1`, `[::1]`, `0.0.0.0`)를 Layer 4에서 강력하게 차단합니다.

**마이그레이션 안내**: `createArk()` 및 `ark.check()` API 문법은 동일합니다. 기존에 프롬프트로 에이전트에게 `ark_approve` 도구 사용 권한을 주었다면, 이제부터는 개발자가 직접 브라우저나 터미널에서 승인해야 합니다.
