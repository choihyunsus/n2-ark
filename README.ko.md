...
> **MCP 서버만으로는 규칙이 물리적으로 강제되지 않습니다.** AI는 `ark_check` 도구를 받고 매 행동 전에 호출하라는 지시를 받지만, 폭주하는 AI는 그냥 건너뛸 수 있습니다. 진짜 강제를 원한다면 에이전트 코드에 **라이브러리로 통합**하세요.
...
> **12개 룰, 138개 정규식 패턴, 10개 위협 카테고리.** 설정 불필요. 체크당 1ms 미만.
...
| 8 | **크립토 마이닝** | `xmrig`, `cpuminer`, `stratum+tcp` | 리소스 도용 |
| 9 | **자기보호 (3중)** | `.n2` 파일 수정, ark 파일 조작, 핵심 파일명 | 방화벽 자체 무력화 방지 |
